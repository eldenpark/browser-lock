...
chrome.extension.sendMessage({ type: "config" }, function (response) {
    ...
    var cmg = {
        ...
        constinit: function constinit() {
            ...
            cmg.valueggesPos = getConfig("gesture", "gesPos");
            ...
            cmg.valuetablistVisual = getConfig("scrollgesture", "tablistVisual");
            ...
            cmg.valuessgsFastSwitch = getConfig("scrollgesture", "fastSwitch");
            cmg.valuessgsReverseFS = getConfig("scrollgesture", "reverseFS");
            ...
            } else if (document.doctype.publicId == "" || document.doctype.publicId == "-//W3C//DTD HTML 4.01//EN" || document.doctype.publicId == "-//W3C//DTD XHTML 1.0 Strict//EN" || document.doctype.publicId == "-//W3C//DTD XHTML 1.1//EN" || document.doctype.publicId == "-//WAPFORUM//DTD XHTML Mobile 1.0//EN") {
        ...
        handleEvent: function handleEvent(e) {
            switch (e.type) {
                case "mousedown":
                    ...
                    /*tablist*/
                    if (cmg.valuescrollgesture && cmg.valuetablist && (cmg.valuetablistkey == "left" && e.button == 0 || cmg.valuetablistkey == "right" && e.button == 2)) {
                        cmg.toTablist = true;
                    }
                    ...
                    && !tostop && (cmg.valuegesture || teststroke)) {
                ...
                case "mouseup":
                    ...
                    cmg.toStrges = false;
                    if (cmg.tablistIng) {
                        chrome.extension.sendMessage({ type: "tablistend", tablistIndex: cmg.tablistId });
                        cmg.toTablist = false;
                ...
                case "dragstart":
                    ...
                    if (!e[cmg.valuedholdkey + "Key"] && !tostop && (cmg.valuedrag || teststroke)) {
                        this.toDrag = true;
                        this._dragstart(e);
                    }
                    break;
                ...
                case "mousewheel":
                    ...
                    if (cmg.toTablist && ud != "") {
                        cmg.tablistIng = true;
                        chrome.extension.sendMessage({ type: "tablist" });
                        ...
                        if (e.wheelDeltaY < 0) {
                            ...
                            //if(!cmg.valuetablistVisual){
                            //   // //chrome.extension.sendMessage({type:"fastWheel", left:true});
                            //   //fireClick(window);
                            //   // return;
                            //}
                            var _tabobj = window.top.document.getElementsByClassName("crxmousetablisttab");
                    ...
                    if (/BackCompat/i.test(document.compatMode)) {
                        var body_check = function body_check() {
                            NotRoot = "";
                            /**/
                            Root = document.body;
                            if (!Root) {
        ...
        _startGuesture: function _startGuesture(e) {
            if (testdrag) {
                return;
            }
        ...
        _progressGesture: function _progressGesture(e) {
            ...
            if (direction != lastDirection) {
                ...
                if (!document.getElementById("infoshowjlgkpaicikihijadgifklkbpdajbkhjo") && (teststroke || cmg.valuegestureui && (cmg.valuedirect || cmg.valuetooltip))) {
                    ...
                    if (cmg.valuetooltip || teststroke) {
                        ...
                        //_tipshow.style.position = "fixed";
                        //_tipshow.setAttribute("style", _tipshow.getAttribute("style") + tipStyleStr);
        ...
        _stopGuesture: function _stopGuesture(e) {
            if (cmg._strokeclear()) {
        ...
        _performAction: function _performAction(e) {
            if (teststroke) {
        ...
        _dragstart: function _dragstart(e) {
            ...
            if (cmg.valuesetdragurl && this._dragType == "text") {
                var tolink;
                if (cmg._seltext.indexOf("http://") != 0 && cmg._seltext.indexOf("https://") != 0 && cmg._seltext.indexOf("ftp://") != 0 && cmg._seltext.indexOf("rtsp://") != 0 && cmg._seltext.indexOf("mms://") != 0 && cmg._seltext.indexOf("chrome-extension://") != 0 && cmg._seltext.indexOf("chrome://") != 0) {
        ...
        _dragprogress: function _dragprogress(e) {
            ...
            /*drag ui ready*/
            if (!document.getElementById("infoshowjlgkpaicikihijadgifklkbpdajbkhjo") && (teststroke || cmg.valuedragui && (cmg.valueddirect || cmg.valuedtooltip))) {
                var _infoshow = document.createElement("div");
                _infoshow.id = "infoshowjlgkpaicikihijadgifklkbpdajbkhjo";
                _infoshow.style.width = Math.min(document.documentElement.scrollWidth, window.innerWidth) + "px";
                _infoshow.style.height = Math.max(document.documentElement.scrollHeight, window.innerHeight) + "px";
        ...
        _dragAction: function _dragAction(e) {
            if (teststroke) {
                optdirect = this._directionChain;
                if (this._dragType) {
        ...
        _strokeclear: function _strokeclear() {
            if (CMGstroketest) {
                return true;
            }
        ...
        _UiReady: function _UiReady() {},
        _gestureaction: function _gestureaction(act) {
            ...
            switch (act) {
                ...
                case "G_ZoomIn":
                    ...
                    chrome.extension.sendMessage({ type: "zoom", zoomIn: true });
                    break;
                case "G_ZoomOut":
                    //var zoomLever = document.body.style.zoom;
                    //if(zoomLever==""){
                    ...
                    //}
                    //document.body.style.zoom = parseFloat(zoomLever) - 0.1;
                    chrome.extension.sendMessage({ type: "zoom", zoomIn: false });
                    break;
                ...
                case "G_tryprev":
                    ...
                    for (var i = matchobj.length - 1; i > 0; i--) {
                        for (var ii = 0; ii < matchwords.length; ii++) {
                            var _matchclass = [];
                            if (!matchobj[i].className) {} else {
                                _matchclass = matchobj[i].className.split(" ");
                            }

                            if (matchobj[i].rel == matchwords[ii] || matchobj[i].rev == matchwords[ii] || matchobj[i].id == matchwords[ii] || _innerobj[i] == matchwords[ii] || matchobj[i].title == matchwords[ii]
    ...
    SmoothScrollByElement.prototype = {
        scroll: function scroll(_x, _y, _duration) {
            var self = this,
                target = this.target,
        ...
        isScrollable: function isScrollable(dir) {
            var self = this,
                target = this.target,
                _target = this._target;
...
chrome.extension.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    if (request.notifitype == "bookmark" || request.notifitype == "isave" || request.notifitype == "isaveback") {
        addnotifibox();
